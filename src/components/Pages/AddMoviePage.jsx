...
import { setFormData, submitMovieForm } from '../../redux/reducer/movieSlice';  // Import necessary actions

const AddMoviePage = () => {
  const dispatch = useDispatch();
  const formData = useSelector((state) => state.movie);  // Get form data from Redux store
  const { loading, error } = formData;  // Loading and error states

  // Handle input field changes
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    dispatch(setFormData({ name, value }));  // Update form data in Redux store
  };
  ...
  const handleCheckboxChange = (e, field) => {
    ...
    dispatch(setFormData({
      name: field,
      value: checked ? [...formData[field], value] : formData[field].filter(item => item !== value),
    }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(submitMovieForm(formData));  // Dispatch the API call action
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto p-8 bg-white shadow-lg rounded-lg space-y-6">
        ...
        <div className="mt-2 space-y-2">
          <div>
            <input
              ...
              className="mr-2"
            />
            <label htmlFor="hollywood" className="text-sm text-gray-700">Hollywood</label>
          </div>
          <div>
            <input
              type="checkbox"
              ...
              className="mr-2"
            />
            <label htmlFor="bollywood" className="text-sm text-gray-700">Bollywood</label>
          </div>
          <div>
            <input
              type="checkbox"
              ...
              className="mr-2"
            />
            <label htmlFor="south" className="text-sm text-gray-700">South</label>
          </div>
        </div>
      </div>
      ...
      {/* Cast Members */}
      <div>
        <label htmlFor="castMembers" className="block text-sm font-medium text-gray-700">Cast Members</label>
        <input
          type="text"
          id="castMembers"
          name="castMembers"
      ...
      <div>
        <label htmlFor="supportingLanguages" className="block text-sm font-medium text-gray-700">Supporting Languages</label>
        <input
          type="text"
      ...
      {/* Thumbnail Image */}
      <div>
        <label htmlFor="thumbnailImage" className="block text-sm font-medium text-gray-700">Thumbnail Image</label>
        <input
          type="file"
          id="thumbnailImage"
          name="thumbnailImage"
        ...
        <div className="mt-2 space-y-2">
          <div>
            <input
              ...
              className="mr-2"
            />
            <label htmlFor="actionGenre" className="text-sm text-gray-700">Action</label>
          </div>
          <div>
            <input
              type="checkbox"
              ...
              className="mr-2"
            />
            <label htmlFor="comedyGenre" className="text-sm text-gray-700">Comedy</label>
          </div>
        </div>
      </div>
