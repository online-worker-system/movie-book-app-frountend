          ...
          premiereArray.map((element, index) => {
            return (
              <SwiperSlide key={index} className='flex flex-col'>
                <img className='w-[100px] object-cover mb-[4px] rounded-md' src={element.img} alt={element.name} />
                <p className='text-white mb-[4px] text-[18px] font-[500]'>{element.name}</p>
                <p className='text-white text-[16px] font-normal'>{element.lang}</p>
              </SwiperSlide>
            );
          })
        }
